...
void	draw(t_fdf field, char **image_buff)
{
	t_dods	point;
	...
	while (point.y < field.hight)
	{
		...
		while (point.x < field.width)
		{
			...
			if (point.x < field.width - 1)
			{
				point.y1 = point.y;
				point.x1 = point.x + 1;
				drawlin(point, &field, image_buff);
			}
...
void	drawlin(t_dods point, t_fdf *field, char **image_buff)
{
	...
	//{
		...
		int pixel = (point.y*field->width*4)+(point.x * 4);
		//write(1, "1", 1);
		//printf("1)pixel = %i\ny = %f\nx = %f\n", pixel, point.y, point.x);
		//*image_buff[pixel] = field->color;
